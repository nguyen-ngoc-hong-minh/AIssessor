...
type AnalyticsData = {
  summary: { visits: number; uniqueVisitors: number; returningVisitors: number; returnRate: number; pageViews: number; bounceRate: number; averageEngagementMs: number; medianEngagementMs: number; longestEngagementMs: number; totalEngagementMs: number; signedInSessions: number };
  ...
  returningProfiles: Array<{ label: string; visits: number; firstVisitAt: number; lastSeenAt: number; location: string; device: string; browser: string; activeTimeMs: number }>;
  topClickedControl: Ranked | null;
  longestStayPage: { path: string; views: number; totalEngagementMs: number; averageEngagementMs: number } | null;
...
export function AnalyticsAdmin() {
  ...
  return <div className="analytics-console">
    ...
    <section className="analytics-summary">
      ...
      <article><RefreshCw /><span>Returning visitors</span><strong>{number(data.summary.returningVisitors)}</strong><small>Visited more than once in this range</small></article>
      <article><ArrowRight /><span>Return rate</span><strong>{data.summary.returnRate}%</strong><small>Returning visitors ÷ unique visitors</small></article>
    ...
    <section className="analytics-highlights" aria-label="Presentation highlights">
      <article className="analytics-highlight-card"><div><MousePointerClick /><span>Most clicked button or link</span></div><strong className="analytics-highlight-metric">{data.topClickedControl ? number(data.topClickedControl.count) : "—"}</strong><p>{data.topClickedControl ? `${data.topClickedControl.label} · clicks in this period` : "No button or link clicks recorded yet."}</p></article>
      <article className="analytics-highlight-card"><div><Clock3 /><span>Page visitors stay on longest</span></div><strong className="analytics-highlight-metric">{data.longestStayPage ? duration(data.longestStayPage.averageEngagementMs) : "—"}</strong><p>{data.longestStayPage ? `${data.longestStayPage.path} · average active time across ${number(data.longestStayPage.views)} page views` : "No active page-time data recorded yet."}</p></article>
    </section>
    <section className="analytics-panel analytics-returning"><header><div><h2>Returning visitor profiles</h2><p>Pseudonymous browser profiles that visited more than once in this selected date range. No names, emails, or raw visitor IDs are shown.</p></div></header><div className="analytics-table analytics-returning-table"><div className="analytics-table-head"><span>Profile</span><span>Visits</span><span>Location</span><span>Device / browser</span><span>First visit</span><span>Last seen</span><span>Active time</span></div>{data.returningProfiles.length ? data.returningProfiles.map((profile) => <div key={profile.label}><strong>{profile.label}</strong><span>{number(profile.visits)}</span><span>{profile.location}</span><span>{profile.device} · {profile.browser}</span><span>{new Date(profile.firstVisitAt).toLocaleDateString()}</span><span>{new Date(profile.lastSeenAt).toLocaleDateString()}</span><span>{duration(profile.activeTimeMs)}</span></div>) : <p className="analytics-empty">No repeat visitors in this date range yet. Choose a longer range to compare returning activity.</p>}</div></section>
