        ...
        <header className="trial-header z-[100] relative w-full flex justify-between items-start">
          ...
          <div className="relative">
            ...
            <div className="hidden sm:flex flex-col items-end gap-0 absolute top-[-32px] right-[-32px] md:right-[-72px] pointer-events-none z-0">
              <div className="bg-[#1A24A9] w-48 h-8" />
              <div className="bg-[#1A24A9] w-32 h-8" />
              <div className="bg-[#1A24A9] w-20 h-8" />
            </div>
      ...
      {phase === "parameters" && <section className="trial-parameters trial-enter" ref={parameterRef}>{activeMode === "monthly" ? <div className="trial-progress monthly-progress"><span className="active">1</span><i /><span>2</span></div> : <div className="trial-progress"><span className="active">1</span><i /><span>2</span><i /><span>3</span></div>}<div className="trial-section-heading"><h2>{activeMode === "monthly" ? "Monthly Workflow" : "One-off Project"}</h2></div>
          <form onSubmit={analyse} className="trial-form">
            {activeMode === "monthly" ? <>
              ...
              <fieldset className="trial-field-wide"><legend>Total budget <InfoTip label="Monthly AI budget">The maximum amount you&apos;re comfortable spending each month on AI subscriptions and usage.</InfoTip></legend><div className="trial-budget-row"><select aria-label="Currency" value={currency} onChange={(event) => { const next = event.target.value as Currency; setCurrency(next); setBudgetChoice(String(suggestedBudgets(next)[2])); }}><option value="USD">USD $</option><option value="AUD">AUD A$</option><option value="VND">VND ₫</option></select>{budgets.map((amount) => <button type="button" aria-pressed={budgetChoice === String(amount)} onClick={() => setBudgetChoice(String(amount))} key={amount}>{budgetLabel(amount, currency)}</button>)}<button type="button" aria-pressed={budgetChoice === "custom"} onClick={() => setBudgetChoice("custom")}>Custom</button></div>{budgetChoice === "custom" && <label className="trial-custom-budget"><span>{currency === "VND" ? "₫" : currency === "AUD" ? "A$" : "$"}</span><input aria-label="Exact monthly AI budget" type="number" min="0" step="any" inputMode="decimal" value={customBudget} onChange={(event) => setCustomBudget(event.target.value)} placeholder="7.50" required /></label>}</fieldset>
            ...
            <fieldset className="trial-field-half"><legend>Total budget <InfoTip label="Project budget">The total you are willing to spend on one-off AI tools or usage to get this project done.</InfoTip></legend><div className="trial-budget-row"><select aria-label="Currency" value={currency} onChange={(event) => { const next = event.target.value as Currency; setCurrency(next); setBudgetChoice(String(suggestedBudgets(next)[2])); }}><option value="USD">USD $</option><option value="AUD">AUD A$</option><option value="VND">VND ₫</option></select>{budgets.map((amount) => <button type="button" aria-pressed={budgetChoice === String(amount)} onClick={() => setBudgetChoice(String(amount))} key={amount}>{budgetLabel(amount, currency)}</button>)}<button type="button" aria-pressed={budgetChoice === "custom"} onClick={() => setBudgetChoice("custom")}>Custom</button></div>{budgetChoice === "custom" && <label className="trial-custom-budget"><span>{currency === "VND" ? "₫" : currency === "AUD" ? "A$" : "$"}</span><input aria-label="Exact AI budget" type="number" min="0" step="any" inputMode="decimal" value={customBudget} onChange={(event) => setCustomBudget(event.target.value)} placeholder="20.00" required /></label>}</fieldset>
