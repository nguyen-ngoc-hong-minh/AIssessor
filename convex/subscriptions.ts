...
export const entitlement = query({ args: hostedAuthArgs, handler: async (ctx, args) => {
  const user = await requireUser(ctx, args); const subscription = await ctx.db.query("subscriptions").withIndex("by_user", (q) => q.eq("userId", user._id)).unique();
  return subscriptionEntitlements(subscription ? { plan: subscription.plan, status: subscription.status } : null);
} });

export const forCurrentUser = query({ args: hostedAuthArgs, handler: async (ctx, args) => { const user = await requireUser(ctx, args); return ctx.db.query("subscriptions").withIndex("by_user", (q) => q.eq("userId", user._id)).unique(); } });

export const upsertVerified = internalMutation({
  args: { userEmail: v.string(), stripeCustomerId: v.string(), stripeSubscriptionId: v.optional(v.string()), stripePriceId: v.optional(v.string()), plan: v.union(v.literal("free"), v.literal("plus"), v.literal("team"), v.literal("enterprise")), status: v.string(), currentPeriodEnd: v.optional(v.number()), cancelAtPeriodEnd: v.boolean() },
  handler: async (ctx, args) => {
    const user = await ctx.db.query("users").withIndex("by_email", (q) => q.eq("email", args.userEmail.toLowerCase())).unique();
