...
export const mine = query({ args: hostedAuthArgs, handler: async (ctx, args) => {
  const user = await requireUser(ctx, args);
...
export const create = mutation({ args: { ...hostedAuthArgs, name: v.string() }, handler: async (ctx, args) => {
  const { name } = args; const user = await requireUser(ctx, args); const subscription = await ctx.db.query("subscriptions").withIndex("by_user", (q) => q.eq("userId", user._id)).unique();
